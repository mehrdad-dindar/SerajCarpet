  ...
  <aside class="max-w-62.5 ease-nav-brand z-990 fixed inset-y-0 my-4 block w-full flex-wrap items-center justify-between overflow-y-auto rounded-2xl border-0 bg-white p-0 antialiased shadow-none transition-transform duration-200 {{ (true ? 'xl:right-0 mr-4 translate-x-full' : 'xl:left-0 ml-4 -translate-x-full ') }}{{--Request::is('rtl')--}} xl:translate-x-0 xl:bg-transparent
    ...
    <div class="h-19.5">
      <i class="absolute top-0 right-0 hidden p-4 opacity-50 cursor-pointer fas fa-times text-slate-400 xl:hidden"
        sidenav-close></i>
      <a class="block px-8 py-6 m-0 text-size-sm whitespace-nowrap text-slate-700" href="{{ route('customer.panel.index') }}" target="_blank">
    ...
    <div class="items-center block w-auto max-h-screen overflow-auto grow basis-full">
      <ul class="flex flex-col pl-0 mb-0">
                  ...
                  <g transform="translate(-1716.000000, -439.000000)" fill="#FFFFFF" fill-rule="nonzero">
                    <g transform="translate(1716.000000, 291.000000)">
                      <g transform="translate(0.000000, 148.000000)">
                        <path class="{{ (Request::is('panel') ? '' : 'fill-slate-800') }} "
        ...
        <li class="w-full mt-4">
          <h6
            class="{{ (true ? 'pr-6 mr-2' : 'pl-6 ml-2') }} font-bold leading-tight uppercase text-size-xs opacity-60">
            Laravel examples</h6>
        </li>
        ...
        <li class="mt-0.5 w-full">
          <a class="py-2.7 text-size-sm ease-nav-brand my-0 mx-4 flex items-center whitespace-nowrap px-4 transition-colors
            ...
            <span
              class="{{ (true ? 'mr-1' : 'ml-1') }} duration-300 opacity-100 pointer-events-none ease-soft">{{ __("Orders") }}</span>
          </a>
        </li>
        ...
        <li class="mt-0.5 w-full">
          <a class="py-2.7 text-size-sm ease-nav-brand my-0 mx-4 flex items-center whitespace-nowrap px-4 transition-colors
            ...
            <span
              class="{{ (true ? 'mr-1' : 'ml-1') }} duration-300 opacity-100 pointer-events-none ease-soft">{{ __("Support") }}</span>
          </a>
        </li>
        ...
        <li class="mt-0.5 w-full">
          <a class="py-2.7 text-size-sm ease-nav-brand my-0 mx-4 flex items-center whitespace-nowrap px-4 transition-colors
            ...
            <span
              class="{{ (true ? 'mr-1' : 'ml-1') }} duration-300 opacity-100 pointer-events-none ease-soft">{{ __("Billing") }}</span>
          </a>
        </li>
        ...
        <li class="mt-0.5 w-full">
          <a class="py-2.7 text-size-sm ease-nav-brand my-0 mx-4 flex items-center whitespace-nowrap px-4 transition-colors
            ...
            <span
              class="{{ (true ? 'mr-1' : 'ml-1') }} duration-300 opacity-100 pointer-events-none ease-soft">{{ __("Customer Club") }}</span>
          </a>
        </li>
        <li class="mt-0.5 w-full">
          <span class="py-2.7 cursor-pointer text-size-sm ease-nav-brand my-0 mx-4 flex items-center whitespace-nowrap px-4 transition-colors
              {{ (Request::is('profile') ? 'shadow-soft-xl rounded-lg bg-white font-semibold text-slate-700' : '') }}">
            <div
              class="{{ (Request::is('profile') ? ' bg-gradient-fuchsia' : '') }} shadow-soft-2xl me-2 flex h-8 w-8 items-center justify-center rounded-lg bg-white bg-center stroke-0 text-center xl:p-2.5">

                <svg xmlns="http://www.w3.org/2000/svg" width="12px" height="12px" viewBox="0 0 512 512"><!--!Font Awesome Free 6.6.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2024 Fonticons, Inc.--><path d="M502.6 278.6c12.5-12.5 12.5-32.8 0-45.3l-128-128c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3L402.7 224 192 224c-17.7 0-32 14.3-32 32s14.3 32 32 32l210.7 0-73.4 73.4c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0l128-128zM160 96c17.7 0 32-14.3 32-32s-14.3-32-32-32L96 32C43 32 0 75 0 128L0 384c0 53 43 96 96 96l64 0c17.7 0 32-14.3 32-32s-14.3-32-32-32l-64 0c-17.7 0-32-14.3-32-32l0-256c0-17.7 14.3-32 32-32l64 0z"/></svg>
