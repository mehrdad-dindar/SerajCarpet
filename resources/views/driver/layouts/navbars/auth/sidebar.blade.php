<aside
    class="max-w-62.5 ease-nav-brand z-20 fixed inset-y-0 my-4 block w-full flex-wrap items-center justify-between overflow-y-auto rounded-2xl border-0 bg-white p-0 antialiased shadow-none transition-transform duration-200 xl:right-0 mr-4 translate-x-full xl:translate-x-0 xl:bg-transparent">
    <div class="h-19.5">
        ...
        <a class="block px-8 py-6 m-0 text-size-sm whitespace-nowrap text-slate-700"
           ...
           target="_blank">
            <img src="{{asset("panel/img/logo-ct.png")}}"
                 ...
                 alt="main_logo"/>
            <span
                class="mr-1 font-semibold transition-all duration-200 ease-nav-brand">Seraj Carpet</span>
        </a>
    </div>
    ...
    <div class="items-center block w-auto max-h-screen overflow-auto grow basis-full">
        <ul class="flex flex-col pl-0 mb-0">
            <li class="mt-0.5 w-full">
                ...
                {{ (Request::is('dashboard') ? 'shadow-soft-xl rounded-lg bg-white font-semibold text-slate-700' : '') }}"
                   href="{{route("driver.panel.index")}}">
                    ...
                    <div
                        class="{{ (Request::is('dashboard') ? ' bg-gradient-fuchsia' : '') }} shadow-soft-2xl flex h-8 w-8 items-center justify-center rounded-lg bg-white bg-center stroke-0 text-center xl:p-2.5">
            ...
            <li class="mt-0.5 w-full">
                <a class="py-2.7 text-size-sm ease-nav-brand my-0 mx-4 flex items-center gap-2 whitespace-nowrap px-4 transition-colors
              {{ (Request::is('dashboard/profile') ? 'shadow-soft-xl rounded-lg bg-white font-semibold text-slate-700' : '') }}"
                   href="{{ route("driver.panel.profile") }}">
                    <div
                        class="{{ (Request::is('dashboard/profile') ? 'bg-gradient-fuchsia ' : '') }}shadow-soft-2xl flex h-8 w-8 items-center justify-center rounded-lg bg-white bg-center stroke-0 text-center xl:p-2.5">
                                    ...
                                    <g id="Icons-with-opacity"
                                       transform="translate(1716.000000, 291.000000)">
                                        <g id="customer-support" transform="translate(1.000000, 0.000000)">
                                            <path
                                                class="{{ (Request::is('dashboard/profile') ? '' : 'fill-slate-800') }}"
            ...
            <li class="mt-0.5 w-full">
                <a class="py-2.7 text-size-sm ease-nav-brand my-0 mx-4 flex items-center whitespace-nowrap px-4 transition-colors {{ (Request::is('dashboard/orders') ? 'shadow-soft-xl rounded-lg bg-white font-semibold text-slate-700' : '') }}"
                   href="{{ route("driver.orders") }}">
                    ...
                    <span
                        class="mr-1 duration-300 opacity-100 pointer-events-none ease-soft">{{ __("Orders") }}</span>
                </a>
            </li>
            <li class="mt-0.5 w-full">
              ...
              {{ (Request::is('tables') ? 'shadow-soft-xl rounded-lg bg-white font-semibold text-slate-700' : '') }}"
                   href="{{ url('tables') }}">
                    <div
                        ...
                        <svg class="fill-slate-800" xmlns="http://www.w3.org/2000/svg" width="12px"
                             height="12px" viewBox="0 0 512 512">
                            <!--!Font Awesome Free 6.6.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2024 Fonticons, Inc.-->
                            <path
